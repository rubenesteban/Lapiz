...
class StatisticsScreenTest {
    ...
    @Test
    fun fruits_showsNonEmptyMessage() = runTest {
        // Given some fruits
        repository.apply {
            createfruit("Title1", "Description1")
            createfruit("Title2", "Description2").also {
                completefruit(it)
            }
        }
        ...
        val expectedActivefruitText = activity.getString(R.string.statistics_active_fruits, 50.0f)
        val expectedCompletedfruitText = activity
            .getString(R.string.statistics_completed_fruits, 50.0f)

        // check that both info boxes are displayed and contain the correct info
        composeTestRule.onNodeWithText(expectedActivefruitText).assertIsDisplayed()
        composeTestRule.onNodeWithText(expectedCompletedfruitText).assertIsDisplayed()
    }
}
