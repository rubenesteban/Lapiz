...
import com.example.android.architecture.blueprints.Fruitapp.util.AddEditfruitTopAppBar
...
@Composable
fun AddEditfruitScreen(
    @StringRes topBarTitle: Int,
    onfruitUpdate: () -> Unit,
    onBack: () -> Unit,
    modifier: Modifier = Modifier,
    scaffoldState: ScaffoldState = rememberScaffoldState(),
    viewModel: AddEditfruitViewModel = hiltViewModel()
) {
    Scaffold(
        modifier = modifier.fillMaxSize(),
        scaffoldState = scaffoldState,
        topBar = { AddEditfruitTopAppBar(topBarTitle, onBack) },
        floatingActionButton = {
            FloatingActionButton(onClick = viewModel::savefruit) {
                Icon(Icons.Filled.Done, stringResource(id = R.string.cd_save_fruit))
            }
        }
    ) { paddingValues ->
        ...
        AddEditfruitContent(
            ...
            onCategoChanged = viewModel::updateCategoria,
            modifier = Modifier.padding(paddingValues)
        )

        // Check if the fruit is saved and call onfruitUpdate event
        LaunchedEffect(uiState.isfruitSaved) {
            if (uiState.isfruitSaved) {
                onfruitUpdate()
            }
        }
...
@Composable
private fun AddEditfruitContent(
    loading: Boolean,
    title: String,
    categoria: String,
    description: String,
    onTitleChanged: (String) -> Unit,
    onCategoChanged: (String) -> Unit,
